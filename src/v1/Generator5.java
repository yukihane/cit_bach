...
class Generator5 extends Generator {
										...
										for (byte v3 = 0; v3 < parametermodel.range[p3]; v3++) {
											for (byte v4 = 0; v4 < parametermodel.range[p4]; v4++) {
												...
												if (constrainthandler.isPossible(tuple) == false) {
													tab.set(p0, v0, p1, v1, p2, v2, p3, v3, p4, v4);
												} else
													numOfTuples++;
											}
										}
	...
	private void initializeUncovTab(ArrayList<Integer>[] uncovTab, QuintTable tab) {
	...
	private ResultOfGenerateOneTest generateOneTest(QuintTable tab, int seedrownum, ArrayList<Integer>[] uncovTab, List<List<Testcase>> tupleSequenceList) {
		// 空のテストケースを1つつくる
		...
		for (int p0 = 0; p0 < numOfParameters - 4; p0++) {
			for (int p1 = p0 + 1; p1 < numOfParameters - 3; p1++) {
				for (int p2 = p1 + 1; p2 < numOfParameters - 2; p2++) {
					for (int p3 = p2 + 1; p3 < numOfParameters - 1; p3++) {
						for (int p4 = p3 + 1; p4 < numOfParameters; p4++) {
							if (tab.get(p0, test.get(p0), p1, test.get(p1), p2, test.get(p2), p3, test.get(p3), p4, test.get(p4)) == false) {
								tab.set(p0, test.get(p0), p1, test.get(p1), p2, test.get(p2), p3, test.get(p3), p4, test.get(p4));
	...
	// 2.20
	// 前の位置をおぼえておく
	private void addUncoveredTuple(Testcase tmp, QuintTable tab, ArrayList<Integer>[] uncovTab) {
		for (int p0 = 0; p0 < numOfParameters - 4; p0++) 
							...
							for (byte v2 = 0; v2 < this.parametermodel.range[p2]; v2++) {
								...
								for (int p3 = p2 + 1; p3 < numOfParameters - 1; p3++)
									for (byte v3 = 0; v3 < this.parametermodel.range[p3]; v3++) {
										if (uncovTab[p3].get(v3) == 0) continue;
										for (int p4 = p3 + 1; p4 < numOfParameters; p4++) 
											for (byte v4 = 0; v4 < this.parametermodel.range[p4]; v4++) {
												if (tab.get(p0, v0, p1, v1, p2, v2, p3, v3, p4, v4) == false) {
													tmp.set(p0, v0);
													tmp.set(p1, v1);
													tmp.set(p2, v2);
													tmp.set(p3, v3);
	...
	private Testcase generateTempTest(Testcase seedrow, QuintTable tab, ArrayList<Integer>[] uncovTab) {
		...
		for (int p1 = 0; p1 < numOfParameters - 3; p1++) {
			for (int p2 = p1 + 1; p2 < numOfParameters - 2; p2++) {
				for (int p3 = p2 + 1; p3 < numOfParameters - 1; p3++) {
					for (int p4 = p3 + 1; p4 < numOfParameters; p4++) {
						if (p == p1 || p == p2 || p == p3 || p == p4) continue;
						if (test.get(p1) < 0 || test.get(p2) < 0 || test.get(p3) < 0 || test.get(p4) < 0) continue;
						if (tab.get(p, test.get(p), p1, test.get(p1), p2, test.get(p2), p3, test.get(p3), p4, test.get(p4)) == false) {
							numOfNewlyCoveredTuples++;
						}
					}
				}
				...
				for (int p2 = p1 + 1; p2 < n - 2; p2++) {
					for (int p3 = p2 + 1; p3 < n - 1; p3++) {
						for (int p4 = p3 + 1; p4 < n; p4++) {
							...
							table[p0][p1][p2][p3][p4] = 
									new QuintList(parametermodel.range[p0],
