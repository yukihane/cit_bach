...
class Testcase {
	...
	void print(BufferedWriter writer, InputFileData inputfiledata) throws IOException {
		for (int i = 0; i < value.length; i++) 
			writer.write((i == 0 ? "": ",") + inputfiledata.parameterList.get(i).value_name.get(value[i]));
		writer.write("\n");
	}
	...
	private boolean superimpose(Testcase tuple) {
		...
		for (int i = 0 ; i < value.length; i++) {
			if (value[i] < 0 || tuple.value[i] < 0) continue;
			if (value[i] == tuple.value[i]) continue;
			return false;
		}
